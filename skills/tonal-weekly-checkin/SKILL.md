...
### 7. Refresh the plan's artifact, if one exists
Invoke the `plan-artifact-refresh` skill (a user-level personal skill) for this plan's `plan_id`. It maintains its own small table of which plans have a published snapshot artifact and is a silent no-op for any plan not in it — don't check for that yourself first, don't mention artifacts if there isn't one, and don't offer to create one. Do this after step 4's `save_training_plan` call (goal-only plans) or any plan write from step 6, since the artifact should reflect the same state just persisted.
